...
//
// Maximum depth is 124. Could probably go to 126 because
...
(
    ...
    reg  [MEM_WIDTH-1 : 0] mem_readdata_0;
    reg  [MEM_WIDTH-1 : 0] mem_readdata_1;
    reg  [MEM_WIDTH-1 : 0] mem_readdata_2;
    reg  [MEM_WIDTH-1 : 0] mem_readdata_3;

    reg  [PAYLOAD_WIDTH-1 : 0] mem_payload_0;
    reg  [PAYLOAD_WIDTH-1 : 0] mem_payload_1;
    reg  [PAYLOAD_WIDTH-1 : 0] mem_payload_2;
    reg  [PAYLOAD_WIDTH-1 : 0] mem_payload_3;
    reg mem_valid;
    reg [1:0] mem_select;
    ...
    // --------------------------------------------------
    localparam OFFSET1 = RAM_BLOCKS_W*ADDR_WIDTH;
    localparam OFFSET2 = 2*RAM_BLOCKS_W*ADDR_WIDTH;
    localparam OFFSET3 = 3*RAM_BLOCKS_W*ADDR_WIDTH;

    assign mem_writedata = sink_payload;

    generate for (i = 0; i < RAM_BLOCKS_W; i=i+1) begin : ram_block_0
        ...
        (
            ...
            .inclock        (clk),
            .outclock       (clk),
            .rdaddress      (rd_ptr[(i+1)*ADDR_WIDTH-3 : i*ADDR_WIDTH]),
            .wraddress      (wr_ptr[(i+1)*ADDR_WIDTH-3 : i*ADDR_WIDTH]),
            .wren           (~wr_ptr[(i+1)*ADDR_WIDTH-1] & ~wr_ptr[(i+1)*ADDR_WIDTH-2]),
            .q              (mem_readdata_0[(i+1)*RAM_WIDTH-1 : i*RAM_WIDTH]),
            .aclr           (1'b0),
            .byteena        (1'b1),
            .inclocken      (1'b1),
            .outclocken     (1'b1),
    ...
    generate for (i = 0; i < RAM_BLOCKS_W; i=i+1) begin : ram_block_1
        ...
        (
            ...
            .inclock        (clk),
            .outclock       (clk),
            .rdaddress      (rd_ptr[OFFSET1+(i+1)*ADDR_WIDTH-3 : OFFSET1+i*ADDR_WIDTH]),
            .wraddress      (wr_ptr[OFFSET1+(i+1)*ADDR_WIDTH-3 : OFFSET1+i*ADDR_WIDTH]),
            .wren           (~wr_ptr[OFFSET1+(i+1)*ADDR_WIDTH-1] & wr_ptr[OFFSET1+(i+1)*ADDR_WIDTH-2]),
            .q              (mem_readdata_1[(i+1)*RAM_WIDTH-1 : i*RAM_WIDTH]),
            .aclr           (1'b0),
            .byteena        (1'b1),
            .inclocken      (1'b1),
            .outclocken     (1'b1),
    ...
    generate for (i = 0; i < RAM_BLOCKS_W; i=i+1) begin : ram_block_2
        ...
        (
            ...
            .inclock        (clk),
            .outclock       (clk),
            .rdaddress      (rd_ptr[OFFSET2+(i+1)*ADDR_WIDTH-3 : OFFSET2+i*ADDR_WIDTH]),
            .wraddress      (wr_ptr[OFFSET2+(i+1)*ADDR_WIDTH-3 : OFFSET2+i*ADDR_WIDTH]),
            .wren           (wr_ptr[OFFSET2+(i+1)*ADDR_WIDTH-1] & ~wr_ptr[OFFSET2+(i+1)*ADDR_WIDTH-2]),
            .q              (mem_readdata_2[(i+1)*RAM_WIDTH-1 : i*RAM_WIDTH]),
            .aclr           (1'b0),
            .byteena        (1'b1),
            .inclocken      (1'b1),
            .outclocken     (1'b1),
    ...
    generate for (i = 0; i < RAM_BLOCKS_W; i=i+1) begin : ram_block_3
        ...
        (
            ...
            .inclock        (clk),
            .outclock       (clk),
            .rdaddress      (rd_ptr[OFFSET3+(i+1)*ADDR_WIDTH-3 : OFFSET3+i*ADDR_WIDTH]),
            .wraddress      (wr_ptr[OFFSET3+(i+1)*ADDR_WIDTH-3 : OFFSET3+i*ADDR_WIDTH]),
            .wren           (wr_ptr[OFFSET3+(i+1)*ADDR_WIDTH-1] & wr_ptr[OFFSET3+(i+1)*ADDR_WIDTH-2]),
            .q              (mem_readdata_3[(i+1)*RAM_WIDTH-1 : i*RAM_WIDTH]),
            .aclr           (1'b0),
            .byteena        (1'b1),
            .inclocken      (1'b1),
            .outclocken     (1'b1),
    ...
    always @(posedge clk) begin
        if (load_mem_reg) begin
            mem_payload_0 <= mem_readdata_0[PAYLOAD_WIDTH-1 : 0];
            mem_payload_1 <= mem_readdata_1[PAYLOAD_WIDTH-1 : 0];
            mem_payload_2 <= mem_readdata_2[PAYLOAD_WIDTH-1 : 0];
            mem_payload_3 <= mem_readdata_3[PAYLOAD_WIDTH-1 : 0];
        end
    end
    ...
    always @(posedge clk) begin
        if (load_output_reg) begin
            src_payload <= (mem_select == 2'b00) ? mem_payload_0 : 
                           (mem_select == 2'b01) ? mem_payload_1 : 
                           (mem_select == 2'b10) ? mem_payload_2 : 
                           mem_payload_3; 
        end
    end
    ...
    always @(posedge clk) begin
        if (load_mem_reg)
            mem_select <= {rd_ptr[ADDR_WIDTH-1], rd_ptr[ADDR_WIDTH-2]};
    end
