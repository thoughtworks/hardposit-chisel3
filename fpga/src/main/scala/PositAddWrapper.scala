...
class PositAddWrapper(totalBits: Int, es: Int) extends Module {
  ...
  private val numbersToRead = 20
  private val maxCount = numbersToRead + 4
  ...
  private val incrementedCounter: UInt = counter + 2.U
  ...
  num2 := Mux(counter < (numbersToRead + 3).U && io.start,io.read_data,0.U)
...
object PositAddWrapper extends App {
  ...
  Driver.execute(optionsManager, () => new PositAddWrapper(16, 1))
}
