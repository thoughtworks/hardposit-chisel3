...
module qsys_top_altera_merlin_axi_master_ni_181_dp4ifgy
#( 
    //----------------
...
always_comb
begin
    total_write_bytecount_wire  = (awlen_q0 + 1) << log2ceil(NUMSYMBOLS);
    total_read_bytecount_wire   = (arlen_q0 + 1) << log2ceil(NUMSYMBOLS);
...
always_comb  
    begin
        ...
        if (AXI_VERSION == "AXI4") begin
            ...
            read_cp_data[PKT_DATA_SIDEBAND_H : PKT_DATA_SIDEBAND_L] = aruser_q0;
        end else begin
        ...
        if(ACE_LITE_SUPPORT == 1) begin
            ...
            read_cp_data[PKT_DOMAIN_H : PKT_DOMAIN_L] = arbar_q0;
            ...
            read_cp_data[PKT_DOMAIN_H : PKT_DOMAIN_L] = 2'b00;    // Not supported. ?? Check if 10 is suitable
            ...
            begin 
                if (count >= 15) begin
                   ERROR_awlock_reserved_value:
                       assert property (@(posedge aclk)
                           disable iff (!aresetn) ( !(awvalid_q0 && awlock_q0 == 2'b11) ));

                   ERROR_arlock_reserved_value:
                       assert property (@(posedge aclk)
                           disable iff (!aresetn) ( !(arvalid_q0 && arlock_q0 == 2'b11) ));
                end
            end
    endgenerate
